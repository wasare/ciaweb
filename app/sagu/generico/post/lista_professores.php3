...
{
	global $pnome, $snome, $so_prof, $hasmore, $limite_list;
	global $like;
	...
	if ( $like != "" )
	{
		...
		$sql = "SELECT" .
           ...
           "    a.nome" .
           " FROM" .
           "    pessoas a, professores b". 
           " WHERE" .
	       "    a.id = b.ref_professor and " .
           "    a.nome ilike '$like'";

		if ( $so_prof == 't' )
		{
			$sql .= " AND EXISTS (SELECT 'x' FROM disciplinas_ofer_prof WHERE ref_professor = a.id)";
		}
...
<form method="post" action="lista_professores.php3" name="selecao">
<div align="center">
<table width="490" border="0" cellspacing="0" cellpadding="2">
	...
	<tr>
		...
		<td><input type="text" name="snome" size="20" maxlength="45"
			value="<?echo($snome)?>"></td>
		<!--<td><input type="checkbox" name="so_prof" value="t" <? echo $so_prof == 't' ? "checked" : ""; ?>></td>
	        <td><font face="Verdana, Arial, Helvetica, sans-serif" size="2">Somente<br>professores</font></td>-->
		</td>
